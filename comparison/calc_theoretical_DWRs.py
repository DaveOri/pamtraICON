'''
calculate DWRs from the normalized mixing ratio (q/N)
'''
#import libraries
import pyPamtra
import numpy as np
#from __triple_freq_plots import triple_freq_plots

     
#define the mixing ratio (NOT sure if that is really kg/kg!!) and number concentration to simulate a wide part of q/N
mix_rat = np.logspace(-8,-4,100)
num_conc = np.ones(mix_rat.shape)*1e2 #the absolute value of N doesnt matter for the DWRs (just q/N

#run the idealized pamtra run
hydrometcount=6  # N of hydrometeors
...
pam.set["radar_nfft"] = 2048
pam.set["radar_no_ave"] = 1000
pam.set["radar_pnoise0"] = -193.
pam.set["radar_max_v"] = 25
pam.set["radar_min_v"] = -25
pam.set["radar_integration_time"] = 5.0
pam.set["verbose"] = 0
pam.set["pyVerbose"] = 0
...
vec_shape=[hydrometcount,1,num_conc.shape[0]]
vec_shape_hyd=[hydrometcount,1,num_conc.shape[0],hydrometcount]
...
pamData["lfrac"] = 1*np.ones(vec_shape[0]) #icon_frland[timeframe]
#pamData["wind10u"] = 1*np.ones(vec_shape[0]) #iconData["u"][0,1]
#pamData["wind10v"] = 1*np.ones(vec_shape[0]) #iconData["v"][0,1]
pamData["hgt"] = np.ones(vec_shape)*np.arange(10.,12000.,(12000.-0.)/(vec_shape[2]))
...
hydro_cmpl 	= 	np.zeros(vec_shape) #allocate
hydro_num_cmpl 	= 	np.zeros(vec_shape)#allocate
hydro_cmpl[:,:,:]	=	mix_rat #iconData[hydromet][0,:]		
hydro_num_cmpl[:,:,:]	=	num_conc #iconData[hydromet][0,:]

for hyd in range(hydrometcount):
	pamData["hydro_q"][hyd,0,:,hyd] = hydro_cmpl[0,0,:]
	pamData["hydro_n"][hyd,0,:,hyd] = hydro_num_cmpl[0,0,:]

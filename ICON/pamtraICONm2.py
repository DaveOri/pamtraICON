...
ICON_filename = 'METEOGRAM_patch001_joyce.nc'
...
#########################################################################
# READ DATA
ICON_file = Dataset(ICON_folder + ICON_filename, mode='r')
#########################################################################
...
pam.writeResultsToNetCDF('ICON_4f_doppler.nc') # SAVE OUTPUT
...
ZeG = np.ma.masked_values(ZeW,-9999)#[:-1,:]
H = pam.p['hgt'][:,0,:]#[:-1,:]
...
plt.gca().xaxis.set_major_formatter(xfmt)
plt.tight_layout()
plt.savefig('ICON_24112015_XKa.png')
plt.close('all')
plt.subplot(2, 1, 1)
...
plt.tight_layout()
#plt.show()
plt.savefig('ICON_24112015_WG.png')
plt.close('all')
...
plt.tight_layout()
#plt.show()
plt.savefig('ICON_24112015_DWRs.png')
plt.close('all')
